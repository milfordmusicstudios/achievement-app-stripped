...
export async function ensureFirstPracticeChallengeAssignment(studioId, studentId) {
  const targetStudioId = String(studioId || "").trim();
  const targetStudentId = String(studentId || "").trim();
  if (!targetStudioId || !targetStudentId) return null;

  const { data, error } = await supabase.rpc("ensure_first_practice_challenge_assignment", {
