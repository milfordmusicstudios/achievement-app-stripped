...
function formatTeacherNames(val) {
...
window.openMultiSelect = function(button, userId, type) {
  ...
  if (type === "roles") {
    ...
    options = allUsers.filter(u => u.roles?.includes("teacher") || u.roles?.includes("admin")).map(u => `${u.firstName} ${u.lastName}`);
  }
...
async function addUser() {
  const { data, error } = await supabase.from("users").insert([{ firstName: "New", lastName: "User", roles: ["student"] }]).select();
  if (!error) {
