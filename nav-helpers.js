...
export async function getAccessFlagsSafely() {
  const utils = await loadUtilsModule();
  if (!utils?.getAccessFlags) return null;
  try {
    return await utils.getAccessFlags();
  } catch (err) {
    console.warn("[nav] access flags load failed; continuing in fallback mode", err);
    return null;
  }
}
